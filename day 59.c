...
struct Node* build(int inorder[], int postorder[], int inStart, int inEnd, int *postIndex) {
    ...
    int i = search(inorder, inStart, inEnd, node->data);

    node->right = build(inorder, postorder, i + 1, inEnd, postIndex);
    node->left = build(inorder, postorder, inStart, i - 1, postIndex);
